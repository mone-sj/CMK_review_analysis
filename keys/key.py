...
class KeywordSent(cmn):
    def __init__(self, site):
        ...
        if self.site=='G':
            ...
            df=db.TB_REVIEW_join_G()
            self.review_join=df[df['DOC_PART_NO']!=0]
        elif self.site=='N':
            self.anal00_partIdList=db.anal00_part_id_list(self.site)
...
# df_naver=db.TB_join_N(anal00_n)
# start_N_join=time.time()
# df_naver=db.TB_REVIEW_join_N()
# print(f'네이버_리뷰 조인시간: {time.time()-start_N_join}')

# anal00_g = db.anal00_G()
# df_glowpick=db.TB_join_G(anal00_g)
# start_G_join=time.time()
# df_glowpick=db.TB_REVIEW_join_G()
# print(f'글로우픽 리뷰 조인시간: {time.time()-start_G_join}')
...
def total(code_list):
    ...
    print(f'{c_proc.name} : {id_cnt} 개 분석')
    print('keys.key.total.7')
    # anal00, review 테이블 join
    # if code_list[0][2]=='N':
    #     print('code_list의 site=N')
    #     df=df_naver
    #     #df=db.TB_REVIEW_join_N()
    #     print('keys.key.total.8')
    # elif code_list[0][2]=='G':
    #     print('code_list의 site=G')
    #     #df=db.TB_REVIEW_join_G()
    #     df=df_glowpick
    #     print('keys.key.total.9')
    ...
    for index, code in enumerate(code_list): # code_list=[part_sub_id,part_id,site]
        ...
        if site_gubun=='N':
            ...
            print('keys.key.total.11-1')
        elif site_gubun=='G':
            df_per_part_id = df[(df['PART_SUB_ID']==sub_id) & (df['PART_ID']==part_id) & df['DOC_PART_NO']!=0]
            print('keys.key.total.11-2')
...
def emo(code_list):
    ...
    if code_list[0][2]=='N':
        ...
        #df=df_naver
        df=db.TB_REVIEW_join_N
        print('keys.key.emo.8')
    elif code_list[0][2]=='G':
        print('code_list의 site=G')
        df=db.TB_REVIEW_join_G()
        #df=df_glowpick
        print('keys.key.emo.9')
...
def keys_multi(site, key_num_cores, df):
    ...
    anal03=total_multi(df,key_num_cores, site)
    anal02=emo_multi(df,key_num_cores, site)
    return anal03, anal02
"""