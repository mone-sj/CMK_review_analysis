...
p.add_argument('--model_load_path', default='./classify/cleanser_classify/', type=str)
p.add_argument('--model_file', default='best_model_cleanser_ver1.pt', type=str)
...
def cleanser_predict(predict_sentence):
    ...
    for batch_id, (token_ids, valid_length, segment_ids, label) in enumerate(test_dataloader):
        ...
        for i in out:
            ...
            if np.argmax(logits) == 0:
                test_eval.append("거품력")
            elif np.argmax(logits) == 1:
                test_eval.append("보습력")
            elif np.argmax(logits) == 2:
                test_eval.append("세정력")
